...
export type ReceiverInput = Pick<Receiver, "label" | "driver" | "serial" | "centerFrequencyHz" | "sampleRateHz" | "gainDb" | "gainSettings" | "ppm" | "enabled" | "role" | "soapyIndex" | "autoTune" | "digitalRecorders" | "analogRecorders" | "dmrRecorders">;
...
export async function createReceiver(input: ReceiverInput): Promise<Receiver> { const response = await fetch("/api/v1/receivers", { method: "POST", headers: { "content-type": "application/json" }, credentials: "include", body: JSON.stringify(input) }); if (!response.ok) throw new Error(response.status === 401 ? "Administrator login required" : await apiError(response)); return response.json() as Promise<Receiver>; }
...
export async function getAuthStatus(signal?: AbortSignal): Promise<AuthStatus> { const response = await fetch("/api/v1/auth/status", { signal }); if (!response.ok) throw new Error(`API returned ${response.status} (${new URL(response.url).pathname})`); return response.json() as Promise<AuthStatus>; }
...
export async function getSession(signal?: AbortSignal): Promise<Session | undefined> { const response = await fetch("/api/v1/auth/me", { credentials: "include", signal }); if (response.status === 401) return undefined; if (!response.ok) throw new Error(`Session check failed (${response.status})`); return response.json() as Promise<Session>; }
...
export type SystemProfile = { id: string; name: string; enabled?: boolean; protocol: string; controlChannelHz?: number; controlChannelsHz?: number[]; nac?: number; frequencyHz?: number; bandwidthHz?: number; modulation?: string; squelchDb?: number; tone?: string; deviationHz?: number; stepHz?: number; dwellMs?: number; sites?: SystemSite[]; receiverId?: string; decodeMdc?: boolean; monitorEncrypted?: boolean; colorCode?: number; timeSlot?: number; contactId?: number; counties?: string[]; townships?: string[]; municipalities?: string[]; localContext?: string };
...
export async function getCallEnrichment(callId: string): Promise<Record<string, unknown>> { const response = await fetch(`/api/v1/calls/${encodeURIComponent(callId)}/enrichment`); if (!response.ok) throw new Error(`Enrichment unavailable (${response.status})`); return response.json() as Promise<Record<string, unknown>>; }
export type IncidentView = { id: string; category: string; headline: string; firstSeen: string; lastSeen: string; callIds: string[]; location?: { label: string; latitude: number; longitude: number; confidence: number }; confidenceState: string; confidence?: number; units: string[] };
export async function getIncidents(): Promise<IncidentView[]> { const response = await fetch('/api/v1/incidents'); if (!response.ok) throw new Error(`Incidents unavailable (${response.status})`); return response.json() as Promise<IncidentView[]>; }
export async function getIncident(id: string): Promise<IncidentView> { const response = await fetch(`/api/v1/incidents/${encodeURIComponent(id)}`); if (!response.ok) throw new Error(`Incident unavailable (${response.status})`); return response.json() as Promise<IncidentView>; }
export async function runAiTask(task: string): Promise<{ task: string; processed: number }> { const response = await fetch(`/api/v1/ai/tasks/${encodeURIComponent(task)}/run`, { method: "POST", credentials: "include" }); if (!response.ok) throw new Error(await apiError(response)); return response.json() as Promise<{ task: string; processed: number }>; }
export async function retryCallEnrichment(callId: string, task: string): Promise<void> { const response = await fetch(`/api/v1/calls/${encodeURIComponent(callId)}/enrichment/${encodeURIComponent(task)}/retry`, { method: "POST", credentials: "include" }); if (!response.ok) throw new Error(await apiError(response)); }
export async function getAiWorkload(hours = 4): Promise<Record<string, unknown>> { const response = await fetch(`/api/v1/ai/workload?hours=${hours}`); if (!response.ok) throw new Error(await apiError(response)); return response.json() as Promise<Record<string, unknown>>; }
export async function getAiModelCapabilities(): Promise<Record<string, unknown>> { const response = await fetch('/api/v1/ai/models/capabilities'); if (!response.ok) throw new Error(await apiError(response)); return response.json() as Promise<Record<string, unknown>>; }
export async function testAiPrompt(task: string, prompt: string, transcript?: string): Promise<Record<string, unknown>> { const response = await fetch('/api/v1/ai/prompts/test', { method: 'POST', headers: { 'Content-Type': 'application/json' }, credentials: 'include', body: JSON.stringify({ task, prompt, transcript }) }); if (!response.ok) throw new Error(await apiError(response)); return response.json() as Promise<Record<string, unknown>>; }
...
export async function purgeCalls(filters: { callIds?: string[]; hours?: number; category?: string; talkgroupId?: number; systemId?: string; deleteAudio?: boolean }): Promise<{ removed: number }> {
...
export async function askOperations(question: string, hours = 4, signal?: AbortSignal): Promise<OperationsAskResponse> {
  const response = await fetch("/api/v1/operations/ask", { method: "POST", signal, headers: { "content-type": "application/json" }, credentials: "include", body: JSON.stringify({ question, hours }) });
  if (!response.ok) throw new Error(response.status === 401 ? "Administrator login required" : `API returned ${response.status} (${new URL(response.url).pathname})`);
