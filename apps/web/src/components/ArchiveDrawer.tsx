...
function enrichmentLists(item: Record<string, unknown>) {
  ...
  return Object.entries(source).filter(([key, value]) =>
    ...
    values: (value as unknown[]).map((entry: unknown) => typeof entry === "string" ? entry : JSON.stringify(entry)),
  }));
}
                    ...
                    <div className="ai-enrichment-list">
                      {Object.entries(call.enrichment).map(([task, value]) => {
                        ...
                        return <div key={task}>
                          ...
                          {enrichmentLists(item).map((list) => <div className="enrichment-list-field" key={list.label}><strong>{list.label}</strong><ul>{list.values.map((entry: string) => <li key={entry}>{entry}</li>)}</ul></div>)}
