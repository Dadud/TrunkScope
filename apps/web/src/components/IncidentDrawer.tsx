...
export function IncidentDrawer({ isOpen, incidents, onClose, onSelectCall }: { isOpen: boolean; incidents: IncidentView[]; onClose: () => void; onSelectCall: (id: string) => void }) {
  ...
  const visible = useMemo(() => filter === "all" ? incidents : incidents.filter((item) => item.category.toLowerCase().includes(filter) || item.confidenceState.toLowerCase() === filter), [filter, incidents]);
  ...
  return <aside className="drawer incident-drawer" aria-label="Incidents">
    ...
    <div className="incident-filters">{["all", "fire", "ems", "law", "traffic", "confirmed", "provisional", "candidate"].map((item) => <button type="button" className={filter === item ? "active" : ""} key={item} onClick={() => setFilter(item)}>{item}</button>)}</div>
